...
public class SofeaLute {
	...
	private SofeaLute(MultiStateConfSpace confSpace, List<StateConfig> stateConfigs, MathContext mathContext, File seqdbFile, MathContext seqdbMathContext, File fringedbFile, long fringedbBytes, boolean showProgress, Parallelism parallelism, double sweepDivisor) {
		...
		try (SeqDB seqdb = openSeqDB()) {
			try (FringeDB fringedb = openFringeDB()) {
				...
				for (MultiStateConfSpace.State state : confSpace.states) {
					...
					fringetx.writeRootNode(state, rootBound, new BigDecimalBounds(stateInfo.blute.factor));
					seqtx.addZSumBounds(state, state.confSpace.makeUnassignedSequence(), rootBound);
				}
				fringetx.commit();
	...
	private class NodeTransaction {
		...
		boolean replace(FringeDB.Transaction fringetx, SeqDB.Transaction seqtx) {
			...
			seqtx.subZSumBounds(state, stateInfo.makeSeq(conf), zbounds);

			// update fringedb and seqdb with the replacement nodes
			for (Node replacementNode : replacementNodes) {
				fringetx.writeReplacementNode(state, replacementNode.conf, replacementNode.zbounds, new BigDecimalBounds(replacementNode.zpath));
				seqtx.addZSumBounds(state, stateInfo.makeSeq(replacementNode.conf), replacementNode.zbounds);
			}

			// add the z values
			for (ZVal zval : zvals) {
				seqtx.addZPath(state, stateInfo.makeSeq(zval.conf), zval.z);
			}
		...
		boolean requeue(FringeDB.Transaction fringetx, SeqDB.Transaction seqtx) {
			...
			fringetx.writeReplacementNode(state, conf, zbounds, new BigDecimalBounds(zpath));
		...
		try (TaskExecutor tasks = parallelism.makeTaskExecutor()) {
			...
			while (true) {
				...
				while (true) {
					...
					synchronized (SofeaLute.this) { // don't race the listener thread
						...
						nodetx = new NodeTransaction(
							...
							fringetx.conf(),
							fringetx.zSumBounds(),
							fringetx.zPathHeadBounds().lower
						);
					}
					...
					tasks.submit(
						...
						(wasExpanded) -> {
							...
							synchronized (SofeaLute.this) { // don't race the main thread
								nodesInFlight[0]--;
