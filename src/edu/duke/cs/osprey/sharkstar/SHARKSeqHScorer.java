...
public class SHARKSeqHScorer implements SeqAStarScorer {
    ...
    public SHARKSeqHScorer(SeqSpace sequenceSpace, SimpleConfSpace fullConfSpace,
        ...
        if(!validScorer())
            throw new UnsupportedOperationException("Incorrect parameters provided to scorer.");
    }

    private boolean validScorer() {
        return seqSpace != null &&
                ((fullConfSpace != null && rigidEnergyMatrix != null && minimizedEnergyMatrix != null)
                || objectives != null);
    }
    ...
    public double calc(SimpleConfSpace fullConfSpace, Sequence seq) {
        return bcalc.freeEnergy(calcBigDecimal(fullConfSpace, seq));
    }

    public BigDecimal calcBigDecimal(SimpleConfSpace confSpace, Sequence seq) {
        ...
        for(SimpleConfSpace.Position pos1 : positions) {
            BigDecimal residueSum = seqOpt.initBigDecimal();
            for (SeqSpace.ResType rt1 : getRTs(pos1, seq)) {
                BigDecimal AASum = BigDecimal.ZERO;
                for (SimpleConfSpace.ResidueConf rc1 : getRCs(pos1, rt1)) {
                    double rotamerSum = minimizedEnergyMatrix.getEnergy(pos1.index, rc1.index);
                    for(SimpleConfSpace.Position pos2 : positions) {
                        if (pos2.index >= pos1.index)
                            continue;
                        double bestPair = confOpt.initDouble();
                        for (SeqSpace.ResType rt2 : getRTs(pos2, seq)) {
                            for (SimpleConfSpace.ResidueConf rc2 : getRCs(pos2, rt2)) {
                                bestPair = confOpt.opt(bestPair, minimizedEnergyMatrix.getEnergy(pos1.index, rc1.index,
                                        pos2.index, rc2.index));
                            }
        ...
        System.out.println("Sum for "+seq+":"+bcalc.freeEnergy(stateSum));
        // add the bound to our estimate of the objective LMFE
        return stateSum;
    ...
    List<SeqSpace.ResType> getRTs(SimpleConfSpace.Position confPos, Sequence seq) {
        ...
        if (seqPos != null) {
            ...
            Integer assignedRT = seq.rtIndices[seqPos.index];
            if (assignedRT != null) {
