...
public class MultiSequenceSHARKStarBound implements PartitionFunction {
    ...
    public static final boolean diveForLeaves = true;
                ...
                ()->{
                    ...
                    try (ObjectPool.Checkout<ScoreContext> checkout = msBound.contexts.autoCheckout()) {
                        ...
                        if(node.getLevel() < seqBound.seqRCs.getNumPos()) {
                            int nextPos = seqBound.getOrder().getNextPos(context.index, seqBound.seqRCs);
    ...
    private void computeRootFringe(SingleSequenceSHARKStarBound singleSequencePfunc, MultiSequenceSHARKStarBound multiSequencePfunc, MultiSequenceSHARKStarNode rootNode){
        final SingleSequenceSHARKStarBound seqBound = singleSequencePfunc;
        final MultiSequenceSHARKStarBound msBound = multiSequencePfunc;

        // set the pfunc's bounds to zero
        seqBound.state.setBoundsWithoutSideEffects(BigDecimal.ZERO, BigDecimal.ZERO);
        final MultiSequenceSHARKStarNode root = rootNode;
    ...
    public void computeForSequenceParallel(int maxNumConfs, SingleSequenceSHARKStarBound sequenceBound){
        ...
        /*
        if (!sequenceBound.nonZeroLower() && runUntilNonZero && false){
        ...
        //computation: ( (!sequenceBound.internalQueue.isEmpty() || !sequenceBound.leafQueue.isEmpty() /*|| loopTasks.isExpecting(0)*/)){ // here, 0 refers to the pfunc computation
        computation: for( int numRoundsWorking = 0; numRoundsWorking < maxNumConfs; /* Don't increment here */ ){
            synchronized(lock) {
                step = Step.None;
                ...
                if (/*lastEps < targetEpsilon ||*/ sequenceBound.getStatus() == Status.Estimated ||
                        ...
                        !sequenceBound.getStatus().canContinue()
                ){
                ...
                if(sequenceBound.internalQueue.isEmpty() &&
                        sequenceBound.leafQueue.isEmpty() &&
                        sequenceBound.state.getNumExpanding() == 0 &&
                        sequenceBound.state.getNumMinimizing() == 0 &&
                        sequenceBound.state.getDelta() > this.targetEpsilon
                ){
                        sequenceBound.setStatus(Status.OutOfLowEnergies);
                    break computation;
                }
            }
            ...
            synchronized(lock) {
                ...
                // If we have nodes in either the leaf or internal queues, process them
                    ...
                    if (freeEnergyLeafError < skipCutoff && freeEnergyLeafError < freeEnergyInternalError) {
                    ...
                    } else if (freeEnergyInternalError < skipCutoff && freeEnergyInternalError <= freeEnergyLeafError) {
            ...
            switch(step) {
                case Energy:{
                    ...
                    sequenceBound.state.numNodesStartedMinimizing.incrementAndGet();
                    loopTasks.submit( () -> {
                ...
                case ExpandInBatches: {
                    ...
                    sequenceBound.state.numNodesStartedExpanding.addAndGet(numNodes);
                ...
                case None: {
                    ...
                    if(sequenceBound.state.getNumExpanding() == 0 && sequenceBound.state.getNumMinimizing() == 0 && sequenceBound.state.getDelta() > this.targetEpsilon){
                        sequenceBound.setStatus(Status.OutOfLowEnergies);
                    }
                    break computation;
                }
    ...
    private void onExpansion(ExpansionResult result){
        ...
        synchronized(result.msBound.lock) {
            ...
            result.sequenceBound.state.numNodesFinishedExpanding.addAndGet(result.numExpanded);
    ...
    private void onMinimization(MinimizationResult result){
        ...
        synchronized(result.msBound.lock) {
            ...
            result.sequenceBound.state.numNodesFinishedMinimizing.incrementAndGet();
    ...
    private MultiSequenceSHARKStarNode drillDown(SingleSequenceSHARKStarBound bound, MultiSequenceSHARKStarBound msBound,
        ...
        try (ObjectPool.Checkout<ScoreContext> checkout = msBound.contexts.autoCheckout()) {
            ...
            int nextPos = bound.getOrder().getNextPos(context.index, RCs);
            ...
            for (int nextRc : RCs.get(nextPos)) {
                ...
                */
                ...
                int nextDesignIndex = bound.getOrder().getNextPos(context.index, bound.seqRCs);
