...
public class ConfSearchTests {
    ...
    public static void testExhaustive(boolean useTriples, boolean useEllipses) {
        ...
        //HMN: For Testing partial emats
        ArrayList<Integer> subsetPos = new ArrayList<>();
        subsetPos.add(1);
        subsetPos.add(2);
        subsetPos.add(3);
        subsetPos.add(4);
        ...
        for (ConfSearch search : searches) {
            ...
            while (true) {
                ...
                RCTuple tup = new RCTuple(pos, rc);
                double origPartE = searchSpace.emat.getInternalEnergy(tup);
                RCTuple tupNew = new RCTuple(newPos, rc);
                double newPartE = newTupMat.getInternalEnergy(tupNew);
                if (origPartE != newPartE) {
                    System.out.println(origPartE + " " + newPartE);
                }
