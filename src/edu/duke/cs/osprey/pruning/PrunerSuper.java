...
public class PrunerSuper {
    ...
    boolean prune(PruningMethod method) {
        ...
        do {
            ...
            for (SuperRCTuple cand : candidates) {
                ...
                {
                    throw new RuntimeException("ERROR: EPIC not currently supported with SuperRCs");
                }
                ...
                if (!pruneMat.isPruned(cand)) {
                    ...
                    if (method.useCompetitor) {
                        ...
                        for (SuperRCTuple competitor : competitorPruneMat.unprunedSuperRCTuplesAtPos(cand.pos)) {
    ...
    public ArrayList<SuperRCTuple> enumerateCandidates(PruningMethod method) {
        ...
        {
            return tupEnum.enumerateUnprunedSuperTuples(method.numPos);
            ...
            return tupEnum.enumerateUnprunedSuperTuples(posTriples);
    ...
    boolean resTypesMatch(SuperRCTuple tup1, SuperRCTuple tup2) {
        ...
        for (int indexInTup = 0; indexInTup < numPosInTup; indexInTup++) {
            ...
            for (int AAIndex = 0; AAIndex < type1.size(); AAIndex++) {
                ...
                if (!AAtype1.equalsIgnoreCase(AAtype2))//this position doesn't match
                {
                    return false;
                }
            }
    ...
    boolean canPrune(SuperRCTuple cand, SuperRCTuple comp, CheckSumType checkSumType, double contELB) {
    ...
    double minInteractionDiff(SuperRCTuple cand, SuperRCTuple comp, int pos, int rc) {
        double E = 0;
        ...
        for (int indexInTup = 0; indexInTup < cand.pos.size(); indexInTup++) {
            ...
            int rc2 = cand.superRCs.get(indexInTup);
            int rc2Comp = comp.superRCs.get(indexInTup);
    ...
    double higherOrderContribGoldstein(SuperRCTuple cand, SuperRCTuple comp, int pos,
    ...
    double higherOrderContribInternal(SuperRCTuple tup, ArrayList<HigherTupleFinder<Double>> htfList) {
        ...
        for (HigherTupleFinder<Double> htf : htfList) {
            for (int posCount = 0; posCount < tup.pos.size(); posCount++) {
                E += htf.getInteraction(tup.pos.get(posCount), tup.superRCs.get(posCount));
    ...
    boolean canPrune(SuperRCTuple cand, CheckSumType checkSumType) {
    ...
    double RCContributionLB(int level, int rc, SuperRCTuple definedTuple) {
        ...
        for (int level2 = 0; level2 < level; level2++) {
            ...
            if (definedTuple.pos.contains(level2) || level2 < level) {//lower-numbered or defined residues
                ...
                if (definedTuple.pos.contains(level2)) {
                    ...
                    int definedRC = definedTuple.superRCs.get(index);
    ...
    public void pruneSteric(double stericThresh) {
        ...
        for (int numBodies = 1; numBodies <= 2; numBodies++) {
            ...
            ArrayList<SuperRCTuple> candList = tupEnum.enumerateUnprunedSuperTuples(numBodies);
            for (SuperRCTuple cand : candList) {

                double E;
                if (numBodies == 1) {
                    E = emat.getOneBody(cand.pos.get(0), cand.superRCs.get(0));
                } else {
                    E = emat.getPairwise(cand.pos.get(0), cand.superRCs.get(0), cand.pos.get(1), cand.superRCs.get(1));
                }
