...
/**
 *
 * A matrix of 1-body, pairwise, etc. energies as polynomials in internal
 * coordinates (EPIC) Together with the EnergyMatrix, this allows precomputation
 * of the complete energy of a continuously flexible system.
 *
 * @author mhall44
 */
public class EPICMatrix extends TupleMatrix<EPoly> {
    ...
    //HMN: This is useful to getting subset EPICMatrices
    public EPICMatrix(TupleMatrix<EPoly> tupMat, ConfSpace confSpace) {
        super(tupMat);
        this.confSpace = confSpace;
    }
    ...
    /**
     * This method updates the epic matrix to include only pairwise terms that 
     * are interacting as defined by the interaction graph
     * We "zero-out" terms by setting the coefficients to zero and removing SAPE
     * terms
     * @param interactionGraph interaction graph that maps positions to true
     * if they interact
     */
                ...
                if (!interactionGraph[i][j]) {
                    for (int rcI = 0; rcI < numRCsPerPos[i]; rcI++) {
                        for (int rcJ = 0; rcJ < numRCsPerPos[j]; rcJ++) {
                            ...
                            zeroOutTerm(getPairwise(i, rcI, j, rcJ));
                        }
                    }
                }
            }
    ...
    /**
     * HMN: We zero out a term by setting its coefficients to zero
     * We also remove any SAPE terms for efficiency
     * @param term 
     */
    public void zeroOutTerm(EPoly term){
        for (int coeff=0; coeff<term.coeffs.length; coeff++){
            term.coeffs[coeff] = 0.0;
        }
        term.sapeTerm = null;
    }
