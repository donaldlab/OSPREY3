...
public class PositionConfSpaceSuper extends PositionConfSpace {
    ...
    public PositionConfSpaceSuper(PositionConfSpaceSuper pcs1, PositionConfSpaceSuper pcs2, int posNum, PruningMatrix pruneMat) {
        ...
        int rcCounter = 0;
        int totalRCCounter = 0;//counts pruned and unpruned to index into pruneMat
        for (int rcIndex1 = 0; rcIndex1 < pcs1.superRCs.size(); rcIndex1++) {
            ...
            for (int rcIndex2 = 0; rcIndex2 < pcs2.superRCs.size(); rcIndex2++) {
                ...
                SuperRCTuple tup = new SuperRCTuple(posNum, totalRCCounter);
                //if not parametrically incompatible and not pruned
                if ((!superRC1.isParametricallyIncompatibleWith(superRC2)) && (!pruneMat.isPruned(tup))) {
                    SuperRC newSuperRC = new SuperRC(superRC1, superRC2, rcCounter);
                    newSuperRCs.add(newSuperRC);
                    rcCounter++;
                } else {//if parametrically incompatible, mark it as pruned
                    pruneMat.setOneBody(posNum, totalRCCounter, true);
                }
                totalRCCounter++;
            }
        }
