...
public class ConfSpace implements Serializable {
     ...
     * @param flexibleRes list of residue numbers to be made flexible (as in PDB
     * file)
    ...
    public ConfSpace(String PDBFile, ArrayList<String> flexibleRes, ArrayList<ArrayList<String>> allowedAAs,
            boolean addWT, boolean addWTRots, boolean contSCFlex, DEEPerSettings dset, ArrayList<String[]> moveableStrands,
    ...
    /**
     * HMN: returns a partial confSpace defined by a subset of the total number
     * of positions
     *
     * @param subsetPosNums subset of total number of positions
     * @return
     */
    public ConfSpace getPartialConfSpace(ArrayList<Integer> subsetPosNums) {
        //Make sure the subset of posNums is sorted
        Collections.sort(subsetPosNums);
        
        //Create deep copy
        //We will build our partialCspace from this
        ConfSpace partialCSpace = (ConfSpace) ObjectIO.deepCopy(this);

        int numPartPosNums = subsetPosNums.size();
        ArrayList<PositionConfSpace> partPosFlexspace = new ArrayList<>();
        ArrayList<ResidueTypeDOF> partMutDOFs = new ArrayList<>();
        
        for (int posNum : subsetPosNums){
            partPosFlexspace.add(partialCSpace.posFlex.get(posNum));
            partMutDOFs.add(partialCSpace.mutDOFs.get(posNum));
        }
        
        partialCSpace.numPos = numPartPosNums;
        partialCSpace.posFlex = partPosFlexspace;
        partialCSpace.mutDOFs = partMutDOFs;
        //Currently we don't change confDOFs at all since they are not used
        
        return partialCSpace;
    }
