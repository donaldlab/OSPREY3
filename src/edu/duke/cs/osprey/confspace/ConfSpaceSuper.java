 ...
 *
 * @author hmn5
 * @author mhall44 This class represents the conformational search space for a
 * design used for GMEC-based design, K*, or anything else we want to do with a
 * conformational space This class just defines the conformational space itself
 * (the molecule + all kinds of flexibility nd possible mutations, and how these
 * are represented as RCs, etc.) This class can be put in an AnnotatedConfSpace
 * to add annotations like what RCs are pruned, what their pairwise energies
 * are, etc.
 */
...
//not need both. If we do want to keep both, just remove the redudancies between the two
    ...
    public ArrayList<PositionConfSpaceSuper> posFlexSuper = new ArrayList<>();
    ...
    public void mergePosition(ArrayList<ArrayList<Integer>> newPosList, PruningMatrix pruneMat) {
        ...
        for (int posIndex = 0; posIndex < newPosFlexSize; posIndex++) {
            ...
            if (posList.size() == 1) {
                int originalPosNum = posList.get(0);
                toMergePosConfSpaceSuper.add(this.posFlexSuper.get(originalPosNum));
            } else if (posList.size() == 2) {
                int originalPosNum1 = Math.max(posList.get(0), posList.get(1));
                int originalPosNum2 = Math.min(posList.get(0), posList.get(1));
                toMergePosConfSpaceSuper.add(this.posFlexSuper.get(originalPosNum1));
                toMergePosConfSpaceSuper.add(this.posFlexSuper.get(originalPosNum2));
            } else {
                throw new RuntimeException("mergePositions can only merge two positions");
            }
            //Merge PositionConfSpaces
            PositionConfSpaceSuper newPosConfSpace = mergePosConfs(toMergePosConfSpaceSuper, posIndex, pruneMat);
            //Add new merged PositionConfSpace
            newPosFlex.add(newPosConfSpace);
        }
        //End: Create new posFlex

        this.posFlexSuper = newPosFlex;
        this.numPos = newPosFlex.size();
    }

    //Given a list of PosConfSupers this method merges them all into one PosConfSuper        
    public PositionConfSpaceSuper mergePosConfs(ArrayList<PositionConfSpaceSuper> posConfSpaceList, int posNum, PruningMatrix pruneMat) {
        ...
        if (posConfSpaceList.size() == 1) {//If size is 1 there is nothing to merge, we just return the position
            ...
            newPosConfSpace = new PositionConfSpaceSuper(posConfSpaceList.get(0), posConfSpaceList.get(1), posNum, pruneMat);
        } else {
            throw new RuntimeException("Can only merge two positions!");
        }
    ...
    public int[] getNumRCsAtPos() {
        ...
        for (int pos = 0; pos < numPos; pos++) {
            numAllowed[pos] = posFlexSuper.get(pos).superRCs.size();
        }
        return numAllowed;
    }
