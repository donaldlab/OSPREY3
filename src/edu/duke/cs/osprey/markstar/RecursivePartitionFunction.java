...
public class RecursivePartitionFunction implements PartitionFunction {
	public final RecursiveAStarTree confSearch;
	...
	public RecursivePartitionFunction(RecursiveAStarTree confSearch, ConfEnergyCalculator ecalc) {
	...
	public void init(double targetEpsilon, BigDecimal stabilityThreshold) {
		...
		lowerBound = new LowerBoundCalculator(confsSplitter.makeStream(), ecalc, targetEpsilon);
		upperBound = new UpperBoundCalculator(confsSplitter.makeStream(), targetEpsilon);
	}
	...
	public void compute(int maxNumConfs) {
		...
		while (true) {
			...
			LowerBoundCalculator.Status lbStatus = lowerBound.energyNextConfAsync((econf) -> {
				...
				synchronized (RecursivePartitionFunction.this) {
	...
	public static class LowerBoundCalculator {
		...
		public double eps = Double.NaN;

		private BoltzmannCalculator boltzmann = new BoltzmannCalculator(PartitionFunction.decimalPrecision);

		public LowerBoundCalculator(ConfSearch tree, ConfEnergyCalculator ecalc, double eps) {
			this.tree = tree;
			this.ecalc = ecalc;
			this.eps = eps;
		}
		...
		public Status energyNextConfAsync(TaskExecutor.TaskListener<EnergiedConf> confListener) {
			/* TODO: Modify this section to work with subtrees.
			 * Likely will have another implementation of the Astar tree that uses nextleaf
			 * instead of nextSubtree
			 */
	...
	public static class UpperBoundCalculator {
		...
		public UpperBoundCalculator(ConfSearch tree, double eps) {
			this.tree = tree;
