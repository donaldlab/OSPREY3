...
public class RecursiveLinkedAStarNode implements ConfAStarNode {
public static class Link implements Comparable<Link> {
	...
	private RecursiveLinkedAStarNode(int level, Link link) {
	...
	public RecursiveLinkedAStarNode() {
		this(0, new Link());
	}
	
	@Override
	public RecursiveLinkedAStarNode assign(int pos, int rc) {
		return new RecursiveLinkedAStarNode(level + 1, new Link(link, pos, rc));
	}
	...
	public void index(ConfIndex index) {
		...
		RecursiveLinkedAStarNode.Link link = this.link;
		while (!(link.isRoot())) {
	...
	public void setUScore(double val) {
		uscore = val;
	}
