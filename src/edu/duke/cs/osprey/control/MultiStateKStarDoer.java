...
public class MultiStateKStarDoer {
	...
	public MultiStateKStarDoer(ConfigFileParser cfp) {
		...
		for(int state=0; state<numStates; state++) {
			...
			checkParamConsistency(state, cfps[state].params, sParams);
	...
	private MultiStateConfigFileParser makeStateCfp(int state, ParamSet sParams) {
		...
		String[] confPaths = { stateKStFile, stateSysFile, stateDEEFile };

		MultiStateConfigFileParser stateCfp = new MultiStateConfigFileParser(confPaths, false);
