            ...
            for (int seqNum = 0; seqNum < numSeqs; seqNum++) {
                ...
                if (!rankByKStar) {
                    ...
                    if (useEPIC && !useTupExp) {
                        stateTree = new ConfTree(searchProb, seqPruneMat, useEPIC);
                        //So we don't have to worry about the ival
                        //TODO: This may be incorrect
                        stateTree.minPartialConfs = true;
                        int conf[] = stateTree.nextConf();
                        if (conf == null) {
                            stateScore[seqNum][state] = Double.POSITIVE_INFINITY;
                        } else {
                            stateScore[seqNum][state] = searchProb.epicMat.minContE(conf) + searchProb.emat.getConstTerm();
                        }
                    } else if (useTupExp) {
                        stateTree = new ConfTree(searchProb.tupExpEMat, seqPruneMat);
                        int conf[] = stateTree.nextConf();
                        if (conf == null) {
                            stateScore[seqNum][state] = Double.POSITIVE_INFINITY;
                        } else {
                            stateScore[seqNum][state] = searchProb.tupExpEMat.getInternalEnergy(new RCTuple(conf)) + searchProb.tupExpEMat.getConstTerm();
                        }
                    } else {
                        stateTree = new ConfTree(searchProb.emat, seqPruneMat);
                        int conf[] = stateTree.nextConf();
                        if (conf == null) {
                            stateScore[seqNum][state] = Double.POSITIVE_INFINITY;
                        } else {
                            stateScore[seqNum][state] = searchProb.emat.getInternalEnergy(new RCTuple(conf)) + searchProb.emat.getConstTerm();
                        }
                    }
                } else {
                    if (useEPIC && !useTupExp) {
