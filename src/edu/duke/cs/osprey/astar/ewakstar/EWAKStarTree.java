...
public class EWAKStarTree extends AStarTree<FullAStarNode> {
    ...
    public EWAKStarTree(int numTreeLevels, EWAKLME objFcn,
...
//        for(EWAKLME constr : constraints){
...
//        }
    ...
    int countGMECsInTree(){
        ...
        for(AStarNode node : getQueue()){
            count += countStateGMECs((EWAKStarNode)node);
        }

        return count;
    }

    int countStateGMECs(EWAKStarNode seqNode){
    ...
    private EnergyMatrix getEnergyMatrix(int state){
        //discrete flexibility w/o LUTE
        //should return lower bound for continuous case, from SearchProblem:
