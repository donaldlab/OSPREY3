...
/**
 *
 * @author PGC: These energy matrices contain bounds between each assigned RC and unassigned RCs.
 */
public class EmatProteinLigandAssignedBoundToUnassigned extends EnergyMatrix {

    ArrayList<Integer> subsetPos;
    EnergyMatrix parent;
    boolean[] isInSubset;
    ArrayList <ArrayList<Double>> bound_RCs_to_US; // Bound of each RC to unassigned residues.

    private double constTerm = 0;
    
    public EmatProteinLigandAssignedBoundToUnassigned(ArrayList<Integer> asubsetPos, 
    		EnergyMatrix emat, ArrayList<ArrayList<Double>> bound_RCs_to_US) {
        this.subsetPos = asubsetPos;
        this.parent = emat;
        this.isInSubset = new boolean[emat.numPos()];
        for (int pos = 0; pos < emat.numPos(); pos++) {
            this.isInSubset[pos] = asubsetPos.contains(pos);
        }
        this.bound_RCs_to_US = bound_RCs_to_US;
    }
    ...
    @Override
    // Returns the internal rotamer of the RC and its bound to unassigned residues.
    public Double getOneBody(int res, int index) {
        if (isInSubset[res]) {
        	int mapToBoundArray = this.subsetPos.indexOf(res); 
            return parent.getOneBody(res, index) + this.bound_RCs_to_US.get(mapToBoundArray).get(index);
        } else {
            return 0.;
        }
    }
