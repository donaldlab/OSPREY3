 ...
 * @author hmn5
 */
public class EmatUnassignedBound extends EnergyMatrix{
    HashMap<Integer,Integer> boundToUnboundPos;
    EnergyMatrix parentBound;
    EnergyMatrix parentUnbound;
    boolean[] isInSubset;

    private double constTerm = 0;
    
    public EmatUnassignedBound(ArrayList<Integer> subsetPos, 
            HashMap<Integer, Integer> bound2Unbound, EnergyMatrix parentBound, EnergyMatrix parentUnbound){
        this.boundToUnboundPos = bound2Unbound;
        this.parentBound = parentBound;
        this.parentUnbound = parentUnbound;
        this.isInSubset = new boolean[parentBound.numPos()];
        for (int pos = 0; pos < parentBound.numPos(); pos++) {
            this.isInSubset[pos] = subsetPos.contains(pos);
        }
    }
    ...
    public Double getOneBody(int res, int index) {
        if (isInSubset[res]) {
            return parentUnbound.getOneBody(boundToUnboundPos.get(res), index);
        } else {
            return 0.;
        }
    }
    ...
    public Double getPairwise(int res1, int index1, int res2, int index2) {
        if (isInSubset[res1] && isInSubset[res2]) {
            return parentBound.getPairwise(res1, index1, res2, index2);
        } else {
            return 0.;
        }
    }
