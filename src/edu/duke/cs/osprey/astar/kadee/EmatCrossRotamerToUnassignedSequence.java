 ...
 * @author PGC
 */
public class EmatCrossRotamerToUnassignedSequence extends EnergyMatrix {

    boolean[][] interactionGraph;
    EnergyMatrix parent;

    private double constTerm = 0;
    private int assignedRCIndex;
    private int assignedPos;
    private int sign =1;

    public EmatCrossRotamerToUnassignedSequence(int assignedPos, int assignedRCIndex, boolean[][] aInteractionGraph, EnergyMatrix emat, boolean positive) {
        this.parent = emat;
        this.interactionGraph = aInteractionGraph;
        this.assignedRCIndex = assignedRCIndex;
    ...
    public Double getPairwise(int res1, int index1, int res2, int index2) {
        if (this.interactionGraph[res1][res2]){
        	if( (res1 == this.assignedPos && index1 == this.assignedRCIndex) || 
        			(res2 == this.assignedPos && index2 == this.assignedRCIndex))
        		return this.sign*parent.getPairwise(res1, index1, res2, index2);
        	else
