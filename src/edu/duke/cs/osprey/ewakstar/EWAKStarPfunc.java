...
/** author: lowegard, based on GradientDescentPfunc.java **/

public class EWAKStarPfunc implements PartitionFunction.WithConfTable {
    ...
    private static class State {
        ...
        boolean energyWindowReached(double targetEnergyWindow) {
            return calcDelta() <= targetEnergyWindow;
        }
    ...
    private double targetEnergyWindow = Double.NaN;
    ...
    private EWAKStarPfunc.State state = null;
    ...
    public EWAKStarPfunc(ConfSearch confSearch, ConfEnergyCalculator ecalc) {
    ...
    @Override
    public void init(double targetEnergyWindow) {
        init(targetEnergyWindow, BigDecimal.ZERO);
    }

    @Override
    public void init(double targetEnergyWindow, BigDecimal stabilityThreshold) {

        if (targetEnergyWindow < 0.0) {
            throw new IllegalArgumentException("Target energy window must be greater than or equal to zero");
        }

        this.targetEnergyWindow = targetEnergyWindow;
        this.stabilityThreshold = stabilityThreshold;

        // init state
        status = Status.Estimating;
        state = new EWAKStarPfunc.State(confSearch.getNumConformations());
    ...
    public void compute(int maxNumConfs) {
        ...
        for (int numConfsEnergied=0; numConfsEnergied<maxNumConfs; /* don't increment here */) {
            ...
            synchronized (this) { // don't race the listener thread
                ...
                if (hasEnergyConfs && ((scoreAheadOfEnergy && energySteeperThanScore) || !hasScoreConfs)) {
                    ...
                    step = EWAKStarPfunc.Step.Energy;

                } else if (hasScoreConfs) {

                    step = EWAKStarPfunc.Step.Score;
        ...
        if (state.energyWindowReached(targetEnergyWindow)) {
