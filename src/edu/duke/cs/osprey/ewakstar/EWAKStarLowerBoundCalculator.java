...
public class EWAKStarLowerBoundCalculator {
	...
	public double currentGMECEnergy = Double.POSITIVE_INFINITY;
	public double curEnergyWindow = Double.NEGATIVE_INFINITY;
	...
	public void setCurrentGMECEnergy(double newEnergy){
		this.currentGMECEnergy = newEnergy;
	}

	public EWAKStarLowerBoundCalculator(ConfSearch tree, ConfEnergyCalculator ecalc) {
		this.tree = tree;
		this.ecalc = ecalc;
	}

	public double getCurEnergyWindow(){
		return curEnergyWindow;
	}
	...
	public Status energyNextConfAsync(TaskExecutor.TaskListener<ConfSearch.EnergiedConf> confListener) {
		...
		TaskExecutor.TaskListener<ConfSearch.EnergiedConf> onEconf = (ConfSearch.EnergiedConf econf) -> {
			...
			synchronized (EWAKStarLowerBoundCalculator.this) {

				// energy calculation done, update pfunc values
				if (!Double.isInfinite(econf.getEnergy())) {
					if(econf.getEnergy() <= currentGMECEnergy){
						setCurrentGMECEnergy(econf.getEnergy());
					}
					curEnergyWindow = econf.getScore() - currentGMECEnergy;
