 ...
 * 
 * THIS VERSION DOES NOT STORE ALL SAMPLE TUPLES (TO SAVE SPACE)
 * 
 * @author mhall44
 */
public class RegTupleFitter2 implements Serializable {
    ...
    //we'll use the sample set to calculate the tuples for each sample.  
    TESampleSet trainingSamples;
    
    public RegTupleFitter2 (TESampleSet trainingSamp, int numTuples, double[] trueVals){
        //sampleTuples.get(s) lists tuple numbers for sample s
        
        trainingSamples = trainingSamp;
        
        //numSamp = sampleTuples.size();
        numSamp = trainingSamples.samples.size();
        numTup = numTuples;
        ...
        AtA = new RealLinearOperator(){
            ...
            public RealVector operate(RealVector rv) throws DimensionMismatchException {
                ...
                for(int s=0; s<numSamp; s++){
                    ArrayList<Integer> sampTup = trainingSamples.calcSampleTuples(trainingSamples.samples.get(s));//sampTuples.get(s);
                    for(int t : sampTup)
                        Arv[s] += rv.getEntry(t);
                }
