...
import edu.duke.cs.osprey.gpu.kernels.DihedralMinimizerPoseKernel;
import edu.duke.cs.osprey.gpu.kernels.ForceFieldKernel;
import edu.duke.cs.osprey.structure.Residue;

public class GpuSurfingLineSearcher implements LineSearcher {
	...
	private DihedralMinimizerPoseKernel poseKernel;
	...
	public void search(ObjectiveFunction f, DoubleMatrix1D x, int d, DoubleMatrix1D mins, DoubleMatrix1D maxs) {
		...
		FreeDihedral dof = (FreeDihedral)mof.getDOFs().get(d);
		efunc = ((GpuForcefieldEnergy)mof.getEfunc(d));
		
		// set the kernel subset and upload the coords
		efunc.getKernel().setSubset(efunc.getSubset());
		efunc.getKernel().updateAndUploadCoordsAsync();
		...
		ForcefieldInteractions.ResidueAtomGroup group = efunc.getKernel().getForcefield().getInteractions().getResidueAtomGroup(res);
		int coordsOffset = efunc.getKernel().getForcefield().getAtomOffset(group);
		...
		// make the pose kernel
		try {
			poseKernel = new DihedralMinimizerPoseKernel(efunc.getKernel().getQueue());
		} catch (IOException ex) {
			throw new Error("can't init dihedral minimizer pose kernel", ex);
		}
		poseKernel.init(efunc.getKernel().getCoords(), dihedralAtomIndices, rotatedAtomIndices);
		
		// init first energy calc
		doEnergy = true;
		energyxd = xd;
		
		// INPUTS: xdmin, xdmax, xd, coords, dihedralAtomIndices, rotatedAtomIndices, newDihedral, doEnergy, energyxd
	...
	private void pose() {
		...
		// set the molecule pose on the gpu
		poseKernel.setDihedralAsync(Math.toRadians(energyxd));
		poseKernel.runAsync();
	}
	...
	private void energy() {
		...
		ForceFieldKernel kernel = efunc.getKernel();
		kernel.runAsync();
		energyfxd = kernel.downloadEnergySync();
	}
