...
class SpecifyChiralityEditor(val prep: MoleculePrep) : SlideFeature {

	override val id = FeatureId("prep.specifychirality")
	...
	override fun gui(imgui: Commands, slide: Slide.Locked, slidewin: SlideCommands) = imgui.run {
		...
		winState.render(
			...
			whenOpen = {
				...
				window("Specify##${slide.name}", winState.pOpen, IntFlags.of(Commands.BeginFlags.AlwaysAutoResize)) {
					...
					prep.partition.forEachIndexed { i, (type, mol) ->
						...
						when (type) {
							MoleculeType.Protein -> {
								...
								checkbox("D Polymer?##${System.identityHashCode(mol)}", prep.isInverted(mol))?.let { isChecked ->
									prep.setInverted(mol, isChecked, slide)
								}
							}
							else -> Unit
						}
