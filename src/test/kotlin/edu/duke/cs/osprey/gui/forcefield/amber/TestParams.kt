...
class TestParams : FunSpec({
	...
	fun assert1cc8AtomTypes(mol: Polymer, types: AmberTypes, atomIndex: AtomIndex) {
	...
	context("1cc8") {
		...
		context("protein") {
			...
			test("chirality doesn't affect amber types") {
				withService {
					val atomIndex = AtomIndex(inverted.atoms)
					val types = inverted.calcTypesAmber(MoleculeType.Protein, atomIndex, ForcefieldName.ff96)
					assert1cc8AtomTypes(inverted, types, atomIndex)
				}
			}

			test("chirality doesn't affect amber mods") {
				withService {
					val atomIndex = AtomIndex(inverted.atoms)
					val types = inverted.calcTypesAmber(MoleculeType.Protein, atomIndex, ForcefieldName.ff96)
					val frcmod = inverted.calcModsAmber(types, atomIndex)
