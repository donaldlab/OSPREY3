...
public class TestSHARKStar {
	...
	private static class Problem {
		...
		final SHARKStar mskstar;
		final ForcefieldParams ffparams;

		public Problem(SHARKStar mskstar, ForcefieldParams ffparams) {
	...
	private static Problem make2RL0PPI(boolean boundedMemory) {
		...
		SHARKStar.State complex = new SHARKStar.State("Complex", confSpaces.complex);

		SHARKStar.LMFE objective = new SHARKStar.LMFE.Builder()
			.addState(complex, 1.0)
			.addState(protein, -1.0)
			.addState(ligand, -1.0)
			.build();
		SHARKStar mskstar = new SHARKStar.Builder(objective)
			.setEpsilon(epsilon)
	...
	private static void check2RL0PPI(SHARKStar mskstar) {

		List<SHARKStar.SequenceInfo> sequences = mskstar.findBestSequences(25);
	...
	private static void check2RL0OnlyOneMutant(SHARKStar mskstar) {

		List<SHARKStar.SequenceInfo> sequences = mskstar.findBestSequences(1);
	...
	private static void check2RL0WithoutWildType(SHARKStar mskstar) {

		List<SHARKStar.SequenceInfo> sequences = mskstar.findBestSequences(2);
	...
	private static void initStates(List<SHARKStar.State> states, ForcefieldParams ffparams, boolean boundedMemory) {
		...
		try (EnergyCalculator ecalc = new EnergyCalculator.Builder(confSpaceList, ffparams)
			...
			for (SHARKStar.State state : states) {
	...
	private static void prepStates(Problem problem, Runnable block) {
		...
		try (EnergyCalculator ecalc = new EnergyCalculator.Builder(confSpaces, problem.ffparams)
			...
			for (SHARKStar.State state : problem.mskstar.states) {
	...
	public static void main(String[] args) {
		double epsilon = 0.99;
		bruteForce("2RL0 PPI", make2RL0PPI(false), 0.1);
	...
	private static void bruteForce(String name, Problem problem, double epsilon) {
		...
		prepStates(problem, () -> {
			...
			for (Sequence sequence : sequences) {
				...
				Map<SHARKStar.State,MathTools.DoubleBounds> stateBounds = new HashMap<>();
				for (SHARKStar.State state : problem.mskstar.states) {
	...
	private static void assertSequence(List<SHARKStar.SequenceInfo> sequences, String seqStr, double objectiveLower, double objectiveUpper) {

		// find the sequence
		SHARKStar.SequenceInfo info = sequences.stream()
	...
	private static void assertSequenceOrder(List<SHARKStar.SequenceInfo> sequences) {
