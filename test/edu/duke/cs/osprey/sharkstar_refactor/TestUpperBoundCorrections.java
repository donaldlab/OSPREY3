...
public class TestUpperBoundCorrections {
    ...
    private static final List<ConfSearch.ScoredConf> confList = new ArrayList<>();
    private static final List<Double> minConfEs = new ArrayList<>();
    ...
    public static void beforeClass() {
        ...
        //Add corrections and respective minimized energies
        confList.add(new ConfSearch.ScoredConf(new int[] {4,2,4,3,2,3,1}, -18.256));
        minConfEs.add(-16.836);
        confList.add(new ConfSearch.ScoredConf(new int[] {4,4,0,3,3,4,1}, -18.013));
        minConfEs.add(-15.497);
        confList.add(new ConfSearch.ScoredConf(new int[] {4,4,0,3,3,3,1}, -16.267));
        minConfEs.add(-15.998);
        confList.add(new ConfSearch.ScoredConf(new int[] {4,4,1,3,3,3,1}, -17.265));
        minConfEs.add(-15.960);
        confList.add(new ConfSearch.ScoredConf(new int[] {4,2,0,3,3,4,1}, -15.997));
        minConfEs.add(-15.419);
    }
     ...
     */
    private ConfAnalyzer makeMinimizerForConfSpace(SimpleConfSpace confSpace){
     ...
     * @return              A ConfAnalyzer object that minimizes
     */
    private ConfAnalyzer makeRigidAnalyzerForConfSpace(SimpleConfSpace confSpace){
    ...
    private EnergyMatrix makeRigidEmatForConfSpace(SimpleConfSpace confSpace){
        // Set up requirements for energy calculation
        Parallelism parallelism = Parallelism.makeCpu(4);
        ForcefieldParams ffparams = new ForcefieldParams();
    ...
    public void testMinimize(){
        ...
        for (int i=0; i < confList.size(); i++){
            ConfAnalyzer.ConfAnalysis analysis = analyzer.analyze(confList.get(i));
            assertThat(analysis.epmol.energy, isAbsolutely(minConfEs.get(i), 0.001));
        }
    ...
    public void testAssignDOFsToFullConfSpace(){
        ...
        for (ConfSearch.ScoredConf conf : confList){
