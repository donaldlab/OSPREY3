...
tasks {
	...
	val generateApiDocs by creating {
		...
		doLast {
			...
			// generate the api documentation into a hugo module in the build folder
			val modDir = buildDir.resolve("doc/api-gen/main")
			modDir.recreateFolder()

			// init the hugo module
			exec {
				commandLine(
					"hugo", "mod", "init", "api-main"
				)
			...
			outJavaDir.resolve("index.html").rename("start.html")

			// tweak the markdown files from the javadoc folder, otherwise hugo get confused
			Files.walk(outJavaDir).use { stream ->
				stream.asSequence()
			...
			// hugo wants to use the index.html url,
			// so rename the index file generated by javadoc to something else
			outKotlinDir.resolve("index.html").rename("start.html")

			// render python docs
			val outPythonDir = contentDir.resolve("python")
			outPythonDir.createFolder()
			val modules = listOf(
				"osprey",
				"osprey.prep",
				"osprey.ccs",
			...
			for ((modulei, module) in modules.withIndex()) {
				pydocMarkdown(module, outPythonDir.resolve("$module.md"), weight = modulei + 1)
			}
		}
	}
