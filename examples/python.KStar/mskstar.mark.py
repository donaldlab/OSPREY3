...
for state in mskstar.states:
	...
	rigidConfEcalc = osprey.ConfEnergyCalculatorCopy(state.confEcalc, rigidEcalc)
	...
	state.pfuncFactory = osprey.PartitionFunctionFactory(state.confSpace, state.confEcalc, state.name, confUpperBoundcalc=rigidConfEcalc)
